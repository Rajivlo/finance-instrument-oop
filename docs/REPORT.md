...
To make the diff tangible, here are the only two files that move in Design B when Monte Carlo is added:

**New file - `MonteCarloOptionPricer.cs` (the entire extension):**

```csharp
/// <summary>
/// Prices a <see cref="EuropeanOption"/> using a 1 000-path
/// risk-neutral GBM Monte Carlo simulation.
/// </summary>
public sealed class MonteCarloOptionPricer : IPricer
{
    ...
    public MonteCarloOptionPricer(int paths = 1_000, int seed = Market.MonteCarloSeed)
    {
        _paths = paths;
        _seed  = seed;
    }
    ...
    public decimal Price(Instrument instrument)
    {
        if (instrument is not EuropeanOption o)
            throw new ArgumentException($"Expected {nameof(EuropeanOption)}.", nameof(instrument));

        var rng      = new Random(_seed);
        double sum   = 0.0;
        double drift = (Market.RiskFreeRate - 0.5 * Market.Volatility * Market.Volatility) * o.TimeToMaturity;
        double diff  = Market.Volatility * Math.Sqrt(o.TimeToMaturity);
        ...
        for (int i = 0; i < _paths; i++)
        {
            ...
            double sT     = Market.SpotPrice * Math.Exp(drift + diff * z);
            double payoff = Math.Max(sT - (double)o.Strike, 0.0);
            sum += payoff * Math.Exp(-Market.RiskFreeRate * o.TimeToMaturity);
        }

        return (decimal)(sum / _paths);
    }
}
```

**Files touched in the rest of the codebase: none.**  
`EuropeanOption.cs` is unchanged. `IPricer.cs` is unchanged. `EuropeanOptionPricer.cs` is unchanged. Calling code selects the strategy at the call site:

```csharp
// analytical reference (unchanged)
IPricer analytical = new EuropeanOptionPricer();

// new strategy - swapped in without touching any existing type
IPricer mc = new MonteCarloOptionPricer(paths: 1_000, seed: 42);

decimal price = mc.Price(option);
```

In Design A the equivalent change requires editing `EuropeanOption.cs` itself - either overloading `Price()` (losing the uniform pricing surface), adding a runtime flag, or introducing a `MonteCarloEuropeanOption` sub-subclass (mixing instrument identity with pricing strategy in the type hierarchy). The Design B diff is one file added; the Design A alternatives each require modifying at least one existing file. That is exactly the OCP intuition, but I now have it grounded in a concrete diff rather than a slogan.
...
- **The comparison only tests one axis of change: a new pricing strategy for an existing instrument type.** Adding an entirely *new instrument type* (e.g. a futures contract) tells the opposite story. Design A requires only one new file - a `Futures` subclass that overrides `Price()`. Design B requires two new files - a `Futures` data class *and* a `FuturesPricer` that implements `IPricer`. For domains where the instrument taxonomy is still evolving but pricing rules are fixed, Design A's single-file extension is the simpler path. The conclusion of this project - "Strategy pays off the moment a second strategy appears for the same data" - holds only for change along the *pricing* axis; along the *instrument* axis the costs are reversed. A fuller study would build a 2×2 change matrix and measure the diff in all four quadrants.
...
- **Class diagrams:** [`docs/design-a.md`](https://github.com/Rajivlo/finance-instrument-oop/blob/main/docs/design-a.md), [`docs/design-b.md`](https://github.com/Rajivlo/finance-instrument-oop/blob/main/docs/design-b.md)
- **Top-level overview and build instructions:** [`README.md`](https://github.com/Rajivlo/finance-instrument-oop/blob/main/README.md)
